...
$adquesto = new Content(\QuestpassVendor\API_URL, '__paste Service UUID here__', new InMemoryStorage(), new CurlHttpClient(), PositioningSettings::factory(PositioningSettings::STRATEGY_UPPER));
try {
    $javascript = $adquesto->javascript([$elementsProvider = new ElementsContextProvider(null, null, 0, $hasActiveCampaigns)]);
...
<div class="questo-should-be-inserted-here"></div>
...
STR;
$content = $adquesto->manualPrepare($originalContent, '<div id="' . $mainQuestElementId . '"></div>', '<div id="' . $reminderQuestElementId . '"></div>');
