...
$options = ['status' => 0, 'subscription' => 0, 'hasActiveCampaigns' => 0];
...
switch ($action) {
    ...
    case 'questo_update_subscription_option':
        $service = new \QuestpassVendor\Adquesto\SDK\Service(new \QuestpassVendor\Adquesto\SDK\ServiceAPIUrl(\QuestpassVendor\API_URL, \QuestpassVendor\SERVICE_UUID), new CurlHttpClient());
        $serviceStatusResponse = $service->fetchStatus();
        if (!$serviceStatusResponse) {
            $response = ['status' => 'ERROR', 'error' => 'Status fetch failed'];
            break;
        }
    ...
    case 'questo_force_update_javascript':
        $adquesto = new Content(\QuestpassVendor\API_URL, \QuestpassVendor\SERVICE_UUID, new InMemoryStorage(), new CurlHttpClient(), PositioningSettings::factory(PositioningSettings::STRATEGY_UPPER));
        try {
            $javascript = $adquesto->requestJavascript(\true);
        } catch (\Exception $e) {
            $response = ['status' => 'ERROR', 'error' => 'Javascript fetch failed: ' . $e->getMessage()];
            break;
        }
